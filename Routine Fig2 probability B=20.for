...
*************************************************
      ...
      real(8), parameter :: xB=20                  ! magnetic field 
cc      
      ...
      character(1), parameter :: JOBVR='V'   ! compute the right generalized eigenvectors
cc
      real(8) Amat(Ndim,Ndim),Bmat(Ndim,Ndim),
     &        VL(LDVL,Ndim),VR(LDVR,Ndim),
     &        ALPHAR(Ndim),ALPHAI(Ndim),BETA(Ndim),WORK(LWORK),
     &        EVAL(Ndim), 
     &        prob1(Ncut),prob2(Ncut),prob3(Ncut),prob4(Ncut),
     &        probN1(Ncut),probN2(Ncut),probN3(Ncut),probN4(Ncut)  
      integer EVALindex(Ndim)
cc      
      end module value
cc      
**********************
...
cc 
      do 100 l=0,2,1 
      write(*,*) l
      write(filename1,'(a2,I2,a4,f3.0,a11)') 'l=', l,', B=',xB, 
     &                                       ', eval1.dat'
      write(filename2,'(a2,I2,a4,f3.0,a11)') 'l=', l,', B=',xB, 
     &                                       ', eval2.dat'      
      write(filename3,'(a2,I2,a4,f3.0,a10)') 'l=', l,', B=',xB,  
     &                                       ', evec.dat'       
...
cc                  
      Amat=0
      do 101 i=1,Ncut-1                            ! Schrodinger equation
         r1=(i-1)*deltar
         r2=r1+deltar
         r=(r1+r2)/2
         Ci=-(1./(2*r)-1./deltar)*76.272/xmr       ! kinetic energy
         Di=-(1./(2*r)+1./deltar)*76.272/xmr 
         Amat(i,Ncut+i)=Ci
         Amat(i,Ncut+i+1)=Di       
cc 
         F1i=38.136*l*l/(xmr*r*r) 
         F2i=2.198D-5*xB*xB*r*r/xmr 
         F3i=(1./xme-1./xmh)*0.05791*l*xB         
...
cc
cc    output the eigenvalues     
      do i=1,Ndim
         write(10,'(I4,100f20.10)') i,ALPHAR(i),BETA(i),
     &                              ALPHAR(i)/BETA(i)
      end do
cc    
...
cc
            Ntemp=EVALindex(i)
            EVALindex(i)=EVALindex(j)
            EVALindex(j)=Ntemp
         end if 
      end do
      end do
      do i=1,4
         write(20,'(I5,f16.8)') EVALindex(i),EVAL(i) 
      end do    
cc        
cc    calculate the probability 
      do i=1,Ncut 
         ri=(i-1)*deltar 
         prob1(i)=ri*VR(i,EVALindex(1))**2
         prob2(i)=ri*VR(i,EVALindex(2))**2 
         prob3(i)=ri*VR(i,EVALindex(3))**2 
         prob4(i)=ri*VR(i,EVALindex(4))**2 
      end do
cc    
cc    normalize the probability 
      call normal(prob1,probN1)
      call normal(prob2,probN2)
      call normal(prob3,probN3)
      call normal(prob4,probN4)      
cc
      do i=1,Ncut
         ri=(i-1)*deltar 
         write(30,'(100f16.8)') ri,
     &         probN1(i),probN2(i),probN3(i),probN4(i)  
      end do
cc           
100   continue 
cc 
...
***********************************
      subroutine normal(ArrayA,ArrayNA)
      use value
      implicit double precision (a-h,o-z)       
      real(8) ArrayA(Ncut),ArrayNA(Ncut)
cc
      sum=0
      do i=1,Ncut
         sum=sum+deltar*ArrayA(i) 
      end do
cc
      do i=1,Ncut
         ArrayNA(i)=ArrayA(i)/sum    
      end do
cc
